...
#### Install Dependencies
    ...
    - `langchain`: Provides `init_chat_model` and, via `langchain-core`, the message types (`HumanMessage`, `SystemMessage`, `BaseMessage`)
    - `langchain-anthropic`: Integration package for the default Anthropic model
...
To use a different LLM provider, install its integration package instead of `langchain-anthropic`, e.g. `langchain-fireworks` for [Fireworks AI](https://fireworks.ai/) - see [Step 7](#step-7-the-main-entry-point).
...
To use a different LLM provider, set the `MODEL` variable and the matching provider API key instead. For example, to use [Fireworks AI](https://fireworks.ai/):
...
The `main()` function is the entry point that ties everything together: instantiate the chat model, build the compiled graph, seed the initial state, stream the graph to completion, and print the result. Streaming (rather than a single `invoke`) keeps the latest state snapshot in hand, so the `finally` block can defensively delete the sandbox if the run aborts before the `cleanup` node executes.

The model is set via the `MODEL` environment variable in `provider:model` format and defaults to Anthropic's Claude model. Any [chat model provider supported by LangChain](https://docs.langchain.com/oss/python/integrations/chat) works - install the provider's integration package and set the `MODEL` variable and the provider's API key in your `.env` file.
...
The graph topology is task-agnostic. To profile a different repository, change the URL in `USER_REQUEST` and rerun. To use a different model, set the `MODEL` environment variable in `provider:model` format (e.g. `fireworks:accounts/fireworks/models/kimi-k2p6`) and set the matching provider API key. To allow more retries per step, raise `max_attempts` in the initial state dict. To run a different analytical workflow entirely, replace `USER_REQUEST`; the plan-and-execute machinery doesn't care what task it's executing.
