...
See also the [Text-to-SQL Agent with the OpenAI Agents SDK and Daytona](https://www.daytona.io/docs/en/guides/openai/text-to-sql-agent-openai-agents-sdk.md) guide for a complete project built on these patterns.
...
To run the examples on a different OpenAI-compatible provider such as [Fireworks AI](https://fireworks.ai/), see [Using a Different Model Provider](#using-a-different-model-provider).
...
The examples above use OpenAI models, but the Agents SDK works with any provider that exposes an OpenAI-compatible Chat Completions endpoint and supports tool calling: build an `AsyncOpenAI` client pointed at the provider's endpoint and pass an `OpenAIChatCompletionsModel` as the agent's `model`. For example, to use [Fireworks AI](https://fireworks.ai/):
    ...
    agent = SandboxAgent(
        ...
        # instructions, default_manifest, capabilities unchanged
    )
    ```

Set `FIREWORKS_API_KEY` in your environment instead of `OPENAI_API_KEY`. The `set_tracing_disabled(True)` call turns off the SDK's tracing export, which uploads to the OpenAI platform by default - with tracing disabled, no `OPENAI_API_KEY` is needed at all.
