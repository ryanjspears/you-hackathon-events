...
To use a different LLM provider, install its integration package instead of `langchain-anthropic`, e.g. `langchain-fireworks` for [Fireworks AI](https://fireworks.ai/) - see [Initialize the Language Model](#4-initialize-the-language-model).
...
To use a different LLM provider, set the `MODEL` variable and the matching provider API key instead - see [Initialize the Language Model](#4-initialize-the-language-model).
