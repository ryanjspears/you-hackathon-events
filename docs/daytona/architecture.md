...
- **Auth0/OIDC provider** authenticates users and services via OpenID Connect. Organizations can also configure [SSO](https://www.daytona.io/docs/en/sso.md) with their own OIDC identity provider. The API enforces organization-level multi-tenancy, where each sandbox, snapshot, and volume belongs to an organization, and access control is applied at the organization boundary
