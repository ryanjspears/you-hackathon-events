...
[Daytona Limits ↗](https://app.daytona.io/dashboard/limits) provide an overview of your organization's [resources](#resources), [sandbox limits](#sandbox-limits), and [rate limits](#rate-limits). Daytona uses a [tier-based](#tiers) system where organizations are placed into tiers based on verification status, with each tier providing access to a specific compute pool and rate limits. For information on spending and wallet management, see [billing](https://www.daytona.io/docs/en/billing.md).
...
Sandboxes count against these limits based on their [lifecycle state](https://www.daytona.io/docs/sandboxes.md#sandbox-lifecycle): stopped, paused, archived, and deleted sandboxes free reserved CPU and memory, while disk quota depends on the sandbox type and state.

## Disk quota

Disk quota and [sandbox billing](https://www.daytona.io/docs/en/billing.md#sandbox-billing) are separate: a sandbox can be billed for reserved disk without counting against your organization's storage limit. The table below details which states occupy disk quota for [container sandboxes](https://www.daytona.io/docs/en/sandboxes.md#create-sandboxes) and [VM sandboxes](https://www.daytona.io/docs/en/sandboxes.md#vm-sandboxes).
...
| Paused    | ✗                     | ✗                                            | Paused VM sandboxes free the disk quota. Supported for <u>[**VM sandboxes**](https://www.daytona.io/docs/en/sandboxes.md#vm-sandboxes)</u> only.                                                                                                                                                                                                                   |
| Archived  | ✗                     | ✗                                            | <u>[**Archiving**](https://www.daytona.io/docs/en/sandboxes.md#archive-sandboxes)</u> moves the container filesystem to object storage, frees the quota, and stops billing. Supported for <u>[**container sandboxes**](https://www.daytona.io/docs/en/sandboxes.md#create-sandboxes)</u> only. VM sandboxes have no archive state because stopping or pausing them already frees the quota. |
...
[Network limits](https://www.daytona.io/docs/en/network-limits.md) are automatically applied based on your organization's billing tier.
:::
