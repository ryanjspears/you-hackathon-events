...
Daytona provides an overview of your organization's [wallet](#wallet) and [spending](#spending). Daytona uses a pay-as-you-go billing model where you are charged based on the resources reserved for your sandboxes. For information on resource quotas, rate limits, and tier-based access, see [limits](https://www.daytona.io/docs/en/limits.md).

## Sandbox billing

Sandboxes are billed for the resources reserved: **vCPU**, **RAM**, and **disk**, depending on the sandbox [lifecycle state](https://www.daytona.io/docs/en/sandboxes.md#sandbox-lifecycle). The table below details which reserved resources and states are billed for [container sandboxes](https://www.daytona.io/docs/en/sandboxes.md#create-sandboxes), [VM sandboxes](https://www.daytona.io/docs/en/sandboxes.md#vm-sandboxes), and [GPU sandboxes](https://www.daytona.io/docs/en/sandboxes.md#gpu-sandboxes).
...
| Stopped                                                 | ✗        | ✗       | ✓        | Billed for reserved disk only. <u>[**Ephemeral sandboxes**](https://www.daytona.io/docs/en/sandboxes.md#ephemeral-sandboxes) </u> and <u>[**GPU sandboxes**](https://www.daytona.io/docs/en/sandboxes.md#gpu-sandboxes)</u> are automatically deleted when stopped, so stopped billing does not apply to them. |
| Paused                                                  | ✗        | ✗       | ✓        | Billed for reserved disk only. The preserved memory state is not billed. Supported for <u>[**VM sandboxes**](https://www.daytona.io/docs/en/sandboxes.md#vm-sandboxes)</u> only.                                                                                                      |
| Archived                                                | ✗        | ✗       | ✗        | Not billed. Supported for <u>[**container sandboxes**](https://www.daytona.io/docs/en/sandboxes.md#create-sandboxes)</u> only.                                                                                                                                                        |
| Deleted                                                 | ✗        | ✗       | ✗        | Not billed. <u>[**Snapshots created from sandbox**](https://www.daytona.io/docs/en/snapshots.md#create-snapshot-from-sandbox)</u> remain billed for storage.                                                                                                                          |

Disk billing and [disk quota](https://www.daytona.io/docs/en/limits.md#disk-quota) are separate: a sandbox can be billed for reserved disk without counting against your organization's storage limit.
